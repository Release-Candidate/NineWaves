...
- [Contributing](#contributing)
  ...
  - [Common Tasks Developing Fabulous-TEMPLATE](#common-tasks-developing-fabulous-template)
...
File a bug report at [Github](https://github.com/Release-Candidate/Fabulous-TEMPLATE/issues/new?assignees=&labels=&template=bug_report.md&title=).
Add a feature request at [Github](https://github.com/Release-Candidate/Fabulous-TEMPLATE/issues/new?assignees=&labels=&template=feature_request.md&title=).
Take a look at the [Issue Tracker at GitHub](https://github.com/Release-Candidate/Fabulous-TEMPLATE/issues)

## Forking the Repository

If you'd like to contribute directly, e.g. better the documentation, add another language or write some source code: fork Fabulous-TEMPLATE by clicking the `Fork` button in the upper right corner of the GitHub project website. Check out your fork of Fabulous-TEMPLATE using the URL from the `Code` button of your fork on Github. The URL should be something like github.com/YOUR_USERNAME/Fabulous-TEMPLATE.git.
...
## Common Tasks Developing Fabulous-TEMPLATE

### Jupyter Notebook

The Jupyter Notebook is located in the project root directory Fabulous-TEMPLATE, named `Fabulous-TEMPLATE.ipynb`. `postBuild` holds the information on how to customize the MyBinder Docker image for the use with F#.
...
In the file [`Pipfile`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/Pipfile) there is a stanza saying
...
in the root directory of Fabulous-TEMPLATE and connect to the running webserver at [http://127.0.0.1:8000](http://127.0.0.1:8000).
...
```yml
    ...
    - Home: index.md
    - Project Links:
        - "GitHub Project Page": "https://github.com/Release-Candidate/Fabulous-TEMPLATE"
        - "Nuget Package": "https://www.nuget.org/packages/Fabulous-TEMPLATE/"
        - "Report a Bug or a Feature Request": "https://github.com/Release-Candidate/Fabulous-TEMPLATE/issues/new/choose"
        - "Issue Tracker at GitHub": "https://github.com/Release-Candidate/Fabulous-TEMPLATE/issues"
    - "Installation & Usage":
...
The Markdown documentation for GitHub are the files [README.md](https://github.com/Release-Candidate/Fabulous-TEMPLATE-calendar/blob/main/README.md) and [CHANGELOG.md](https://github.com/Release-Candidate/Fabulous-TEMPLATE-calendar/blob/main/CHANGELOG.md) in the project root directory.
...
configured in the file [`.config/dotnet-tools.json`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/.config/dotnet-tools.json)
...
There are 4 projects in Fabulous-TEMPLATE:
...
- `TestFabulous-TEMPLATEGlyph.fs` - tests the module `Fabulous-TEMPLATEGlyph`
...
- NuGet packages generated by the target `Packages` are saved to the directory `nuget`, set using the expression
  `packageOutputPath` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx).
- The test results of the targets `Tests` and `TestsDeb` are saved as  `.xml` files in the
  directory `test_results`, set using the expression  `testOutputPath` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx).
- The coverage report `coverage.xml` of the targets `TestsCoverage` and `TestsCoverageDeb`
  ...
  in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx).
- The output of the `Publish` target is copied to the directory `bin`, set using the expression  `exeOutPath` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx).
...
The Fake script [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx) defines the following targets:
...
- `Build` and `BuildDeb` build the configured projects
   ...
   `src` and `tests`. The expression `buildProjs` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx) defines
   the projects to build.
- `Docs` generates the HTML documentation using MkDocs. The HTML is created in the directory `sites`.
  The output directory is configured in the MkDocs config file [`mkdocs.yml`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/mkdocs.yml)
- `Lint` runs FSharpLint on all configured projects. Default is all projects, `lintProjs` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx)
...
- `Tests` and `TestsDeb` build and run all test projects in the directory `tests` using `dotnet test`.
  ...
  Default is testing all projects in the directory `tests`, `testProjs` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx) defines the projects to run `Tests` and `TestsDeb` on.
- `TestsCoverage` and `TestsCoverageDeb`  build and run all tests projects and generate a coverage report using AltCover.
  ...
  Default is testing all projects in the directory `tests`, `testProjs` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx) defines the projects to run `TestsCoverage` and `TestsCoverageDeb` on.
- `Publish` runs `dotnet publish` on the configured projects, default is all in `src`. The
  ...
  and be the same as the newest one in [CHANGELOG.md](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/CHANGELOG.md).
  `publishProjs` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx) defines the projects to run `Publish` on.
...
- `Packages` builds the NuGet packages of all projects using a release build. The
  build is done with the release configuration, the version **must** be given as argument on the command line
  and be the same as the newest one in [CHANGELOG.md](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/CHANGELOG.md).
  `packageProjs` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx) defines the projects to run `Packages` on.
...
- `VERSION_STRING` is the version to use for the target, **must** be the same as the first (newest)
  version in the file [`CHANGELOG.md`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/CHANGELOG.md). Targets that need `VERSION_STRING` are
...
Run `dotnet publish` on the configured projects (`publishProjs` in [`build.fsx`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/build.fsx))  setting the
version to 6.2.53
...
using a GitHub action. Used in the workflows [linux_test.yml](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/.github/workflows/linux_test.yml),
[osx_test.yml](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/.github/workflows/osx_test.yml) and [windows_test.yml](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/.github/workflows/windows_test.yml)
...
- `create_packages.yml` creates and uploads the NuGet packages, runs `dotnet publish` and
  ...
  the source with a release tag of the form `v?.?.?` (**must be the same version as the newest in [CHANGELOG.md](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/CHANGELOG.md)**).
  Appends the newest entry in [CHANGELOG.md](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/CHANGELOG.md) to the release - script [`scripts/get_changelog.sh`](https://github.com/Release-Candidate/Fabulous-TEMPLATE/blob/main/scripts/get_changelog.sh)
  See the [latest release](https://github.com/Release-Candidate/Fabulous-TEMPLATE/releases/latest) as an example
...
The badges of the workflows are linked in the section [Badges](https://github.com/Release-Candidate/Fabulous-TEMPLATE#badges )
